...
**Streaming reasoning**: During SSE streaming, `reasoning_content` deltas from OpenAI-compatible providers (DeepSeek R1, etc.) are parsed as `ReasoningToken` events and displayed in real-time. Reasoning tokens are stored into `History.reasoningContent` directly and also appended to `History.content` with `isThinking=true` so ChatScreen can render them as a thinking bubble. When the first answer token arrives, `content` is reset to just the answer text (the reasoning is already in `reasoningContent`) and `isThinking` flips to `false`, collapsing the thinking block into the standard collapsible "Thinking" section above the streaming answer. Anthropic and Gemini do not yet emit reasoning tokens in the streaming path.
